...
STATUS_COMMANDS = {"status", "/status", "info", "/info", "статус", "/статус"}
...
def build_admin_help_message() -> str:
    return (
        "Доступные команды:\n\n"
        "Статус - сведения о суффиксе, группах и режиме автопубликации\n"
        ...
        "Любое иное сообщение от администратора сразу публикуется во все целевые группы. "
        "Фотографии необходимо отправлять как фотографии VK. "
        "Последовательность \\n в тексте и суффиксе преобразуется в перенос строки."
    )


def build_admin_keyboard(awaiting_pattern_input: bool = False) -> str:
...
def build_status_message(
    ...
    return (
        "Статус бота:\n"
        "Режим: мгновенная публикация без подтверждения\n"
        f"Суффикс: {suffix_preview}\n"
        f"Группы: {targets_preview}\n\n"
        "Любое новое сообщение администратора сразу отправляется во все указанные группы."
    )
...
def handle_pattern_command(
    ...
    if not new_pattern:
        ...
        notify_admin(
            ...
            keyboard=build_admin_keyboard(awaiting_pattern_input=True),
        )
        return True
...
def publish_message(
    ...
    post_text = compose_post_text(text, suffix)
    success_lines: list[str] = []
    error_lines: list[str] = []

    LOGGER.info(
        "Publishing admin message immediately: text_len=%s attachments=%s suffix_len=%s",
        len(text),
        len(attachments),
        len(suffix),
    )
...
def handle_admin_message(
    ...
    if normalized_text in STATUS_COMMANDS:
        notify_admin(
            bot_api,
            admin_id,
            build_status_message(store.load(), targets),
            keyboard=build_admin_keyboard(),
        )
        return False
    ...
    if first_token in PATTERN_COMMANDS:
        ...
        return awaiting_pattern_input

    attachments = collect_photo_attachments(bot_api, message)

    LOGGER.info(
        "Admin publish payload prepared: text_len=%s attachments=%s",
        len(text),
        len(attachments),
    )
    ...
    while True:
        try:
            ...
            for event in longpoll.listen():
                ...
                try:
                    awaiting_pattern_input = handle_admin_message(
                        bot_api=bot_api,
                        admin_id=sender_id,
                        store=pattern_store,
                        targets=targets,
                    ...
                    notify_admin(
                        ...
                        keyboard=build_admin_keyboard(awaiting_pattern_input=awaiting_pattern_input),
                    )
